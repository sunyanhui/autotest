...
from element.element_shop_supply_order import ElementSupplyOrder
from action.basepage import BasePage
from common import output
import traceback, time, re

class ShopSupplyOrder(BasePage, ElementSupplyOrder):
    u'''
    联盟店供货订单
    '''
    ...
    def del_order(self,*args, **kwargs):
        u'''
        删除供货订单
        '''
        try:
            ...
            self.driver.find_elements_by_link_text(u"删除")[0].click()
            self.driver.switch_to.default_content()
            self.find_element(self.ok_btn).click()
            self.driver.switch_to.frame('iframe')
            assert u'采购单删除成功' in self.driver.page_source
        except:
            print traceback.format_exc()
            return output.error_auto(self.driver)
        else:
            return output.pass_user_defined(self.driver, "删除供货订单成功")
        finally:
            self.driver.switch_to.default_content()
