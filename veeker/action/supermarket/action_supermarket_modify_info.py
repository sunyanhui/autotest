...
from element.supermarket.element_supermarket_modify_info import *
from action.basepage import BasePage
from common import output
import time

class SupermarketModifyInfo(BasePage):
    u'''
    修改超市基本信息页面对象
    '''
    ...
    def modify_info(self, **w):
        ...
        try:
            ...
            find_element(modify_supermarket_info_link).click()
            driver.switch_to_frame('iframe')

            #输入公司名称
            find_element(supermarket_name).clear()
            find_element(supermarket_name).send_keys(w['name'])

            #输入公司简称
            find_element(supermarket_short_name).clear()
            find_element(supermarket_short_name).send_keys(w['shot_name'])
