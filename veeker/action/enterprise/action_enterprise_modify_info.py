...
from element.element_enterprise_modify_info import ElementModifyInfo
from action.basepage import BasePage
from common import output


class EnterpriseModifyInfo(BasePage, ElementModifyInfo):
    u'''
    修改超市基本信息页面对象
    '''
    ...
    def modify_info(self,name,shot_name,manage_type,manage_range,province,city,country,
        ...
        try:
            ...
            find_element(self.modify_enterprise_info_link).click()
            driver.switch_to_frame('iframe')

            #输入公司名称
            find_element(self.enterprise_name).clear()
            find_element(self.enterprise_name).send_keys(name)

            #输入公司简称
            find_element(self.enterprise_short_name).clear()
            find_element(self.enterprise_short_name).send_keys(shot_name)
